...
class ExtendShelfRental extends Component
{
    ...
    {
        $old_rental_end = $this->shelfRental->rental_end;
        ...
        $invoice = Invoice::create([
            ...
            'invoice_identifier' => "ShelfRentalExtension_" . now()->format('Y-m-d-H-i-s'),
        ]);
        ShelfRentalExtension::create([
            'shelf_rental_id' => $this->shelfRental->id,
            'user_id' => auth()->id(),
            'previous_end_date' => $old_rental_end,
        ...
        $productAddUrl = url('/shelf-rental/'.$this->shelfRental->id); 
        $bookingDetailsUrl = url('/admin/shelf-rentals/' . $this->shelfRental->id); 
        $adminEmail = Setting::where('key', 'admin_email')->pluck('value')->first();
        Notification::route('mail', $adminEmail)->notify(new AdminBookingCreatedNotification( $this->shelfRental, $bookingDetailsUrl));

        try {
                $user = auth()->user();
               $user->notify(new ShelfBookingConfirmation($user, $this->shelfRental, $productAddUrl));
        ...
        $message = 
            ...
            '<table>
                <tr><td><strong>Regalnummer:</strong></td><td>' . $this->shelfRental->shelf->floor_number . '</td></tr>
                <tr><td><strong>Neuer Mietzeitraum:</strong></td><td>' . 
                Carbon::parse($this->shelfRental->rental_start)->format('d.m.Y') . ' bis ' . 
                Carbon::parse($this->shelfRental->rental_end)->format('d.m.Y') . '</td></tr>
        ...
        $this->loadRental();
        $this->checkAvailableExtensions();
        Log::info('Eine Miete wurde erfolgreich verlängert!');
    }
