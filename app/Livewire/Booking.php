...
class Booking extends Component
{
    ...
    public function mount()
    {
        ...
        $this->periodsSettings = Setting::where('type', 'period')
        ->pluck('value')
        ->toArray();
    }
    ...
    {
        // Standardpreise basierend auf der Periode
        ...
        $rentalStart = isset($this->startDate) ? Carbon::parse($this->startDate) : null;
    
        // Verfügbare Boni abrufen
        $validBonuses = $this->getValidBonuses($rentalStart);
    
        if ($validBonuses->isNotEmpty()) {
            // Den besten Bonus auswählen (höchste Reduzierung)
            $this->bestBonus = $validBonuses->sortByDesc('value')->first();
    
            // Berechnung je nach Bonus-Typ
            if ($this->bestBonus->type === 'percentage') {
                $this->discountedPrice = $this->totalPrice * (1 - ($this->bestBonus->value / 100));
            } elseif ($this->bestBonus->type === 'amount') {
                $this->discountedPrice = max(0, $this->totalPrice - $this->bestBonus->value);
            }
    ...
    private function getValidBonuses($rentalStart)
    {
        $currentDate = Carbon::now()->format('Y-m-d');
    
        return Bonus::where('status', 1) // Nur aktive Boni
        ->where('periods', 'LIKE', '%'.(int)$this->period.'%')
        ->where(function ($query) use ($currentDate) {
                $query->whereNull('valid_from')
                    ->orWhere('valid_from', '<=', $currentDate); // Buchungszeitraum: Muss bereits gültig sein
            })
            ->where(function ($query) use ($currentDate) {
                $query->whereNull('valid_until')
                    ->orWhere('valid_until', '>=', $currentDate); // Buchungszeitraum: Darf noch nicht abgelaufen sein
            })
            ->where(function ($query) use ($rentalStart) {
                $query->whereNull('booking_start_from')
                    ->orWhere('booking_start_from', '<=', $rentalStart); // Mietbeginn muss in diesem Zeitraum liegen
            })
            ->where(function ($query) use ($rentalStart) {
                $query->whereNull('booking_start_until')
                    ->orWhere('booking_start_until', '>=', $rentalStart); // Mietbeginn darf noch nicht überschritten sein
            })
            ->where(function ($query) {
                if (Auth::check()) {
                    // Bestehender Kunde -> Muss 'existing' oder 'all' sein
                    $query->whereIn('customer_requirement', ['existing', 'all']);
                } else {
                    // Neuer Kunde -> Muss 'new' oder 'all' sein
                    $query->whereIn('customer_requirement', ['new', 'all']);
                }
            })
            ->get();
    }
