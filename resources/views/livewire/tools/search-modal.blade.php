<div x-data="{ openSearchMenu: @entangle('openSearchMenu') }" @keydown.window.escape="openSearchMenu = false"  class="h-full">
    ...
    <div @click.prevent="() => { openSearchMenu = !openSearchMenu; }"  class="flex h-full w-16 transition-all duration-300 ease-in-out" >
        <div x-cloak class="flex  pt-2 px-2 mr-8 rounded-t-full transition-all duration-300 absolute h-min bottom-3.5 bg-gray-100 pb-1.5"  :class="openSearchMenu ? 'text-secondary bg-gray-200 border-t border-x  border-gray-300 translate-y-[1px] rounded-b-0 !bottom-0 !pb-0' : 'text-gray-500 translate-y-0 rounded-b-full'">
            <svg 
