<div>
    <section class="text-gray-600 body-font overflow-hidden py-24">
        <div class="container mx-auto px-4 my-8">
            <div class="relative bg-blue-50 border border-blue-200 text-blue-900 px-6 py-5 rounded-md shadow-sm">
                <!-- Icon -->
                <div class="absolute top-5 left-5">
                    <svg class="w-6 h-6 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M18 10A8 8 0 1 1 2 10a8 8 0 0 1 16 0zm-9 4a1 1 0 1 0 2 0 1 1 0 0 0-2 0zm.25-8.75a.75.75 0 0 1 1.5 0v5a.75.75 0 0 1-1.5 0v-5z" clip-rule="evenodd" />
                    </svg>
                </div>

                <!-- Textblock mit Padding für Icon -->
                <div class="pl-10">
                    <h2 class="text-xl font-semibold mb-2">Abonnements – in Vorbereitung</h2>
                    <p class="mb-4">
                        Aktuell befinden wir uns mit <strong>Regulierungs-Check</strong> noch in der Aufbau- und Testphase. Unsere Plattform lebt von echten Erfahrungsberichten und fundierten Auswertungen – und genau davon gibt es zum jetzigen Zeitpunkt noch nicht genug, um bereits professionelle Abonnement-Modelle anzubieten.
                    </p>
                    <p class="mb-4">
                        Unser Ziel ist es, mit zunehmender Datenbasis auch erweiterte Funktionen zur Verfügung zu stellen: Geplant sind Abonnements, die Zugriff auf <strong>vertiefte Auswertungsanalysen</strong>, <strong>erweiterte Filtermöglichkeiten</strong> sowie <strong>detaillierte Einblicke</strong> in branchenspezifische Trends und Vergleichsdaten bieten. Diese Funktionen richten sich vor allem an Nutzer:innen mit professionellem Interesse – etwa <strong>Versicherungsmakler</strong> – und nicht an reguläre Endverbraucher.
                    </p>
                    <p class="mb-4">
                        Sobald ausreichend Bewertungen gesammelt und systematisch ausgewertet wurden, werden diese Zusatzfunktionen Schritt für Schritt freigeschaltet. Sie ermöglichen unter anderem Segmentierungen nach Versicherungsart, Schadenhöhe, Bearbeitungsdauer oder Zufriedenheitsverlauf über Zeit.
                    </p>
                    <p>
                        Wir bitten daher noch um etwas Geduld – und freuen uns über jede eingereichte Bewertung, die hilft, unsere Datenbasis zu erweitern. Denn je mehr echte Erfahrungen geteilt werden, desto fundierter und aussagekräftiger werden die künftigen Analyse-Tools im Abonnementbereich.
                    </p>
                </div>
            </div>
        </div>
    </section>
</div>
