<button   {!! $attributes->merge(['class' => 'transition-all duration-100 inline-flex items-center p-3 justify-center bg-green-100  text-base font-medium text-center text-green-900 border border-green-300 rounded-full aspect-square hover:bg-green-200 focus:ring-4 focus:ring-green-100 ']) !!} x-data="{ isClicked: false }" 
    @click="isClicked = true; setTimeout(() => isClicked = false, 100)"
    style="transform:scale(1);"
    :style="isClicked ? 'transform:scale(0.9);' : ''">
    <svg class="text-green-600 fill-green-600 h-8 aspect-square" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 521.4157 547.7315">
